...
# Car Information
@app_employee.route('/forms/employee')
@login_required
def car_registration():
  return render_template('/form_employee.html',tab_id="1")

@app_employee.route('/forms/maintenance_reg/apply',methods=['GET', 'POST'])
@login_required
def db_car_register():
  data = request.form
  car_register(data) 
...
@app_employee.route('/forms/maintenance_search/apply',methods=['GET', 'POST'])
@login_required
def car_search():
