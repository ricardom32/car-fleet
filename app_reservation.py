...
app_reservation = Blueprint('app_reservation',__name__)

# Car Information
@app_reservation.route('/forms/reservation')
@login_required
def car_registration():
  return render_template('/form_maintenance.html',tab_id="1")

@app_reservation.route('/forms/maintenance_reg/apply',methods=['GET', 'POST'])
@login_required
def db_car_register():
  data = request.form
  car_register(data) 
...
@app_reservation.route('/forms/maintenance_search/apply',methods=['GET', 'POST'])
@login_required
def car_search():
